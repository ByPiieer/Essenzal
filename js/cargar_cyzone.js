function cargarCyzone(jsonUrl, { showSection = true, respectItemVisibility = true } = {}) {
  const section = document.getElementById('cyzone');
  const grid    = document.getElementById('cyzone-grid');

  if (!section || !grid) {
    console.warn('Falta la sección/grid de Cyzone');
    return;
  }
  ...
  const buildWhatsappMessage = (item) => {
    ...
    return tpl
      ...
      .replaceAll('{marca}',  item.marca  ?? 'cyzone');
  };
  ...
  fetch(jsonUrl)
    ...
    .then(data => {
      ...
      if (!items.length) {
        grid.innerHTML = `
          <div class="col-span-full text-center text-brandDark/70 py-10">
            No hay productos de cyzone disponibles.
          </div>`;
        return;
      }
      ...
      items.forEach(p => {
        ...
        try {
          // Log útil en dev
          // console.log('[cyzone] imagen declarada:', srcImagen, '-> abs:', new URL(srcImagen, location.href).href);
        } catch (_) {}
        ...
        card.innerHTML = `
          ...
          <div class="p-4 flex flex-col gap-3 flex-1">
            <header class="text-center">
              ${p.codigo ? `<p class="text-[11px] tracking-wide text-gray-300/70 mb-1 select-none">${p.codigo} · ${p.marca ?? 'cyzone'}</p>` : ''}
    ...
    .catch(err => console.error('Error cyzone:', err));
}
